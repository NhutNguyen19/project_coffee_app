...
import project.duan.qlybancafe.repository.InvalidatedTokenRepository;
...
public class AuthenticationService implements IAuthenticationService {
    ...
    public AuthenticationResponse refreshToken(RefreshRequest request) throws ParseException, JOSEException {
        ...
        var account =
                accountRepository.findByUsername(username).orElseThrow(() -> new AppException(ErrorCode.UNAUTHORIZED));
        var token = generateToken(account);
        return AuthenticationResponse.builder()
                .token(token)
    ...
    public String resetPassword(ResetPasswordRequest request) {
        PasswordEncoder passwordEncoder = new BCryptPasswordEncoder(12);
        Account account = accountRepository.findByPhoneAndUsername(request.getPhone(), request.getUsername())
